...
|---|---|---|
| FR-01 | Must | The system shall expose conversations.replies, conversations.history, users.list, and conversations.list endpoints |
...
| FR-08 | Should | The system shall support optional rate limiting that mirrors Slack's documented tier limits |
| FR-09 | Should | The system shall return HTTP 429 with a Retry-After header when rate limited |
...
- [ ] FR-01: The server responds to /api/conversations.replies, /api/conversations.history, /api/users.list, /api/conversations.list
...
3. Should the server support POST endpoints for write operations?
